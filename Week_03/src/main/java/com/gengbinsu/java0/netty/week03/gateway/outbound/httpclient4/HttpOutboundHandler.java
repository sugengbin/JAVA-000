...
import com.gengbinsu.java0.netty.week03.gateway.router.strategy.RoundRobinRouter;
...
public class HttpOutboundHandler {
	...
	public HttpOutboundHandler(String backendUrls) {
		// backend servers
		for (String url : backendUrls.split(",")) {
			this.backendUrls.add(url.endsWith("/") ? url.substring(0, url.length() - 1) : url);
		}
		...
		IOReactorConfig ioConfig = IOReactorConfig.custom().setConnectTimeout(1000).setSoTimeout(1000)
				.setIoThreadCount(cores).setRcvBufSize(32 * 1024).build();

		httpclient = HttpAsyncClients.custom().setMaxConnTotal(40).setMaxConnPerRoute(8)
				.setDefaultIOReactorConfig(ioConfig).setKeepAliveStrategy((response, context) -> 6000).build();
		httpclient.start();
	}

	public void handle(final FullHttpRequest fullRequest, final ChannelHandlerContext ctx) {
		final String url = RoundRobinRouter.getInstance().route(backendUrls) + fullRequest.uri();
	...
	private void fetchGet(final FullHttpRequest inbound, final ChannelHandlerContext ctx, final String url) {
		...
		for (Entry<String, String> entry : inbound.headers().entries()) {
